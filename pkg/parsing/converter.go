...
func (c *unstructuredConverter) fromUnstructured(o *unstructured.Unstructured) (runtime.Object, error) {
	fromGVK := o.GetObjectKind().GroupVersionKind()
	newObj, err := c.scheme.New(fromGVK)
	if err != nil {
		return nil, err
	}
	if err := c.scheme.Convert(o, newObj, nil); err != nil {
		return nil, fmt.Errorf("failed to convert object from unstructured %s: %w", fromGVK.String(), err)
	}
	newObj.GetObjectKind().SetGroupVersionKind(fromGVK)
	return newObj, nil
}
